...
export default function DashboardSidebar({
  activeTab = "overview",
  setActiveTab,
  isMobile = false, // 🔑 add this flag
  onClose,          // 🔑 for closing on mobile
}: {
  activeTab: string
  ...
  const menuItems = [
    ...
    { id: "subscriptions", label: "Subscriptions", icon: <Play className="h-5 w-5" /> },
    { id: "billing", label: "Billing", icon: <FileText className="h-5 w-5" /> },
    { id: "KycCompliance", label: "KYC & Compliance", icon: <ShieldCheck className="h-5 w-5" /> },
  ...
  return (
    <div
      className={`relative flex flex-col h-full bg-background/60 backdrop-blur-sm border-r border-brand-blue/20 transition-all duration-300
    ...
    >
      ...
      {/* Logo */}
      <div className="p-4 border-b border-brand-blue/20 flex items-center gap-2">
        ...
        {!collapsed &&         <span>
          <span className="text-lg font-bold text-brand-blue">Subverse</span>
            ...
            <li key={item.id}>
              <button
                ...
                onClick={() => {
                  ...
                  if (isMobile && onClose) onClose() // 🔑 close drawer on mobile select
                }}
              >
                ...
                {activeTab === item.id && !collapsed && (
                  <motion.div
                    className="w-1 h-5 bg-brand-blue rounded-full ml-auto"
